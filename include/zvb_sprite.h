...
typedef enum {
    ...
    SPRITE_FLIP_Y       = 1 << 2,
    SPRITE_FLIP_X       = 1 << 3,
    /* The upper nibble is the palette, check the SPRITE_PALLETE(p) macro */
...
typedef enum {
    SPRITE_OPTION_NONE          = 0,
    SPRITE_OPTION_32PX          = 1 << 1, // 16x32 sprite, uses tile+1 for tall sprites
...
typedef struct {
    ...
    uint16_t options;   /*!< Options for the sprite, check `gfx_sprite_options` enum for more details*/
} gfx_sprite;
